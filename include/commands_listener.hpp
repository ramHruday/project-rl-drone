...
sensor_msgs::BatteryState current_battery_g;
sensor_msgs::Image current_image_g;
...
geometry_msgs::TwistStamped current_velocity_g;
ros::Time image_sub_begin;
ros::Time image_sub_end;
...
// get armed state
void state_b(const sensor_msgs::BatteryState::ConstPtr &msg)
{
    current_battery_g = *msg;
}

void velocity_s(const geometry_msgs::TwistStamped::ConstPtr &msg)
{
    current_velocity_g = *msg;
}

void printToConsole(int msg)
{
    ROS_INFO("Local position %d", msg);
}
...
int resetCommandCounter()
{
    command_counter = 0;
}

std::map<std::string, std::array<int, 2>> getCommandMap()
{
    return CommandMap;
}
...
double getImageRate()
{
    return image_sub_end.toSec() - image_sub_begin.toSec();
}
...
std::vector<double> get_current_posing()

{

    float x = current_pose_g.pose.pose.position.x;
    float y = current_pose_g.pose.pose.position.y;
    float z = current_pose_g.pose.pose.position.z;
    std::vector<double> b = {x, y, z};
    float d = sqrt(x * x + y * y + z * z);

    ROS_INFO("total distance from curr pos d: %f", d);
    return b;
}
...
int wait4Land()
{
    ROS_INFO("Waiting for user to set mode to Landed");
    while (ros::ok() && current_state_g.mode != "LAND")
    {
        ros::spinOnce();
        ros::Duration(0.01).sleep();
    }
    ...
    {
        ROS_INFO("Mode set to land. Mission starting");
        return 0;
    }
    else
    {
...
int arm()
{
    ...
    {
        ROS_INFO("Arming failed with %d", arm_request.response.result);
        return -1;
    }
}
...
void topicCallback(const std::string &name)
{
    try
    {
        int val = 0;
        ...
        {
            val = CommandMap[name][0];
            CommandMap.erase(name);
            std::array<int, 2> value = {val + 1, (int)current_battery_g.percentage};
            CommandMap.insert(std::make_pair(name, value));
        }
        else
        {
            std::array<int, 2> value = {1, (int)current_battery_g.percentage};
            CommandMap.insert(std::make_pair(name, value));
        }
    }
...
void image_s(const sensor_msgs::Image::ConstPtr &msg)
{
    if (image_sub_begin.is_zero())
    {
        image_sub_begin = ros::Time::now();
    }
    topicCallback("/webcam/image_raw");
    image_sub_end = ros::Time::now();
}

int init_publisher_subscriber(ros::NodeHandle controlnode, bool listenToCommands = true)
{
    ...
    bat_sub = controlnode.subscribe<sensor_msgs::BatteryState>((ros_namespace + "mavros/battery").c_str(), 10, state_b);
    vel_sub = controlnode.subscribe<geometry_msgs::TwistStamped>((ros_namespace + "/mavros/local_position/velocity").c_str(), 10, velocity_s);
    image_sub = controlnode.subscribe<sensor_msgs::Image>((ros_namespace + "/webcam/image_raw").c_str(), 10, image_s);
    ...
    if (listenToCommands)
    {
        ...
        try
        {
            ...
            {
                int index = std::distance(topic_infos.begin(), it);

                const ros::master::TopicInfo &info = *it;
                auto name = info.name;
                ROS_INFO("THe topic is %s", (ros_namespace + name.c_str()).c_str());
                if (!in_array(name, excludedTopics))
                {
                    ros::spinOnce();
                    rate.sleep();
                    boost::function<void(const topic_tools::ShapeShifter::ConstPtr &)> callback;
                    callback = [name](const topic_tools::ShapeShifter::ConstPtr &msg) -> void
                    {
                        topicCallback(name);
                    };
                    ros::Subscriber subscriber = controlnode.subscribe(name, 10, callback);
                    topic_subs.push_back(subscriber);
                }
            }
        }
        catch (const std::exception &e)
        {
            std::cerr << e.what() << "cout error" << '\n';
            std::cout << "cout error" << std::endl;
        }
    }
