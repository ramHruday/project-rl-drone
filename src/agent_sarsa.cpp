...
public:
    ...
    Drone(ros::NodeHandle nh, int victim_position[3], int inc, int h, int al, float g, float e)
    {
   ...
   */
    ...
    */
    ...
    std::pair<int, int> get_action(std::string state)
    {
        ...
        int random_action_index = (rand() % 3) + 1;
        if (r < epsilon)
        {
            action = actions[random_action_index];
        }
    ...
    */
    void set_Q_value(std::string state, std::string next_state, int action, int reward, int next_action)
    {
        ...
        Q_s_a = Q_s_a + alpha * (reward + gamma * QTable[next_state][next_action] - Q_s_a);
...
int main(int argc, char **argv)
{
    ...
    std::vector<std::string> discrete_state_space = drone.discrete_state_space;
    while (iterations < thresh)
    {
        ...
        for (size_t i = 0; i < discrete_state_space.size() - 1; ++i)
        {
            ...
            if (i < states.size() - 1)
            {
                ...
                drone.set_Q_value(current_state, state_maker(i, j + 1), current_action, reward, next_state);
            }
        }
        iterations++;
    }
