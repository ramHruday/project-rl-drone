...
void detection_cb(const darknet_ros_msgs::BoundingBoxes::ConstPtr &msg)
{
    ...
    std::cout << std::fixed << avg_image_p_time << std::endl;
}
...
void generate_report(TimedWaypointList waypointList, float final_battery, int commandCount, std::string report_name, float total_d)
{
    ...
    myfile << "<tfoot><tr style='font-weight: bold'><td></td>"
           ...
           << "% </ th></ tr> </tfoot>";
    myfile << "</table>";
    myfile << "<div style='margin-top:2px'> Number Commands published during this trip " << commandCount << "</div>";
    myfile << "<div style='margin-top:2px'> Average image processing time " << avg_image_p_time << "</div>";
